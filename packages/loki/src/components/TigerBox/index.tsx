...
const TigerBox: React.FC<ITigerBox> = ({
  ...
  const turnsTime = config?.turnsTime || 0; //配置动画时间
  const circleNumber = config?.circleNumber || 0; //配置滚动圈数
  ...
  const Rotating = (prizeId, callback) => {
    ...
    options = Array.from(
      ...
      return {
        el: dom.querySelector(".loki-wrap"), //指向奖项元素的父级
        location: prizeId * itemHeight, // 奖品滚动到指定的位置
        rollTimes: 2000 + Math.random() * 500 + i * 500 + 1000 * circleNumber, // 来回滚动圈数
        height: prizeList.length * itemHeight, // 总的高度
        duration: duration + i * 2000 + turnsTime, // 动画时间，毫秒数
        isFinished: false,
      };
    });
  ...
  return (
    <div
      ...
      style={boxStyle}
    >
      <div className="loki-tigerbox--container" style={innerStyle}>
        <img
        ...
        {[{}, {}, {}].map((ite, index) => {
          return (
            <div className="loki-tigerbox-wrap" key={`ite-${index}`}>
              <div className="loki-tigerbox-content">
                <div className="loki-wrap">
                  {[...prizeList, ...prizeList]?.map((item, index) => {
                    return (
                      <div
                        className="loki-item"
                        key={`loki-${index}`}
                        style={itemStyle}
                      >
                        <div className="loki-item-image">
                          <img className="loki-item-img" src={item.image} />
                        </div>
                      </div>
                    );
                  })}
